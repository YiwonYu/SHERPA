...
class PanoGenerator(PanoBase):
    ...
    def get_cn(self, unet):
        ...
        self.enable_xformers_if_available(cn, 'controlnet')
        cn.enable_gradient_checkpointing()
        return cn, (list(cn.parameters()), 0.1)

    def enable_xformers_if_available(self, module, module_name):
        if not getattr(self.hparams, "use_xformers", True):
            return
        ...
        try:
            ...
            print(f"xformers attention disabled for {module_name}: {error}")
            if hasattr(module, "disable_xformers_memory_efficient_attention"):
    ...
    def load_branch(self, add_lora, train_lora, add_cn):
        ...
        self.enable_xformers_if_available(unet, 'unet')
