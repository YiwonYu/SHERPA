...
class Mp3dDataset(PanoDataset):
    def load_split(self, mode):
        ...
        if self.config['pers_dir'] is not None:
            """
            Dataset should be structured as:
            pers_dir
            > {name1}.{jpg,png} # image 1
            > {name1}.txt       # caption 1
            > {name2}.{jpg,png} # image 2
            > {name2}.txt       # caption 2
            ...
            """
            if self.config['pers_only']:
                new_data = []
            idx = 0
            suffix = "_mllama.txt" if self.config['use_mllama_caption'] else ".txt"
            for fname in tqdm(os.listdir(self.config['pers_dir'])):
                if fname.endswith(".png") or fname.endswith(".jpg"):
                    if os.path.exists(os.path.join(self.config['pers_dir'], f'{fname[:-4]}{suffix}')):
                        if self.config['num_pers'] is not None and idx >= self.config['num_pers']:
                            break
                        try: 
                            _ = self.load_prompt(os.path.join(self.config['pers_dir'], f'{fname[:-4]}{suffix}'))
                        except:
                            continue
                        new_data.append({
                            'img_path': os.path.join(self.config['pers_dir'], fname),
                            'prompt_path': os.path.join(self.config['pers_dir'], f'{fname[:-4]}{suffix}'),
                            'is_pers': True,
                        })
                        idx += 1
    ...
    def get_data(self, idx):
        ...
        if 'is_pers' in data and data['is_pers']:
            return data
