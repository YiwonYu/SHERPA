...
class PanoDataset(torch.utils.data.Dataset):
    def __init__(self, config, mode='train'):
        ...
        if not self.config['gt_as_result'] and self.result_dir is not None:
            ...
            new_data = set([(item['scene_id'], item['view_id']) for item in self.data]) & set(results)
            if len(new_data) != len(self.data):
                ...
                self.data = [{'scene_id': s, 'view_id': v} for (s, v) in new_data]
                self.data.sort()
    ...
    def __getitem__(self, idx):
        ...
        def process_equi(equi, normalize):
            ...
            imgs = []
            # for t, p in zip(theta, phi):
            idx = np.random.randint(len(list(zip(theta, phi))))
            t, p = list(zip(theta, phi))[idx]
        ...
        if 'is_pers' in data and data['is_pers']:
            ...
            if img.mode == 'RGBA':
                img = img.convert("RGB")
            img = cv2.resize(np.array(img), (self.config['pers_resolution'], self.config['pers_resolution']), interpolation=cv2.INTER_AREA)
            if self.result_dir is None:
                img = (img.astype(np.float32)/127.5)-1
            imgs = rearrange(np.stack([img]), 'b h w c -> b c h w')
            data['images'] = imgs
            data['prompt'] = [self.load_prompt(data['prompt_path'])]
            data['pano_prompt'] = ""
        ...
        # load pano prompt
        if 'pano_prompt' not in data and 'pano_prompt_path' in data:
...
class PanoDataModule(L.LightningDataModule):
    def __init__(
            ...
            pers_dir: str = None,
            num_pers: int = None,
            pers_only: bool = False,
            use_mllama_caption: bool = False,
            ):
        super().__init__()
