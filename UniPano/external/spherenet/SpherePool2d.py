...
class SphereMaxPool2d(nn.MaxPool2d):
  ...
  def forward(self, x):
    ...
    x = F.grid_sample(x, grid, align_corners=False, mode='nearest')  # (B, in_c, H*Kh, W*Kw)
...
class SphereAvgPool2d(nn.AvgPool2d):
  """
  kernel_size: (H, W)
  """
  ...
  def forward(self, x):
    ...
    x = F.avg_pool2d(x, kernel_size=self.kernel_size, stride=self.kernel_size)
