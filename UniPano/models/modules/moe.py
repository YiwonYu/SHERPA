...
class SparseMoeLoRA(nn.Module):
    ...
    """
    def __init__(self, embed_dim, rank, alpha, num_experts=16, num_experts_per_tok=2, sync_expert=False):
        super().__init__()
        ...
        self.experts = nn.ModuleList([LoRALinearLayer(embed_dim, embed_dim, rank, alpha) for _ in range(num_experts)])
        self.gate = MoEGate(embed_dim=embed_dim, num_experts=num_experts, num_experts_per_tok=num_experts_per_tok)
        self.n_shared_experts = 2
        if self.n_shared_experts is not None:
            self.shared_experts = LoRALinearLayer(embed_dim, embed_dim, rank, alpha)
