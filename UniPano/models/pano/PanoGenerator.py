...
from diffusers.models.lora import LoRACompatibleConv#, LoRAConv2dLayer
from diffusers.models.resnet import Downsample2D, Upsample2D
import torch.nn as nn
from .utils import vae_set_use_conv
from .modules import AdaptableLoRAAttnProcessor
...
class PanoBase(WandbLightningModule):
    ...
    def get_pers_prompt(self, batch):
        if self.hparams.copy_pano_prompt:
            prompts = sum([[p] * (batch['images'].shape[1] if 'images' in batch else 1) for p in batch['pano_prompt']], [])
        else:
...
class PanoGenerator(PanoBase):
    ...
    def __init__(
        ...
        if ckpt_path is not None:
            ...
            # state_dict = self.process_state_dict(state_dict)
    ...
    def process_state_dict(self, state_dict):
        ...
        for k, v in state_dict.items():
            if "_orig_mod" in k and "vae" not in k:
                new_k = k[:k.index("_orig_mod")] + k[k.index("_orig_mod")+len("_orig_mod")+1:]
                res_state_dict[new_k] = v
            else:
                res_state_dict[k] = v
        del state_dict
    ...
    def load_branch(self, add_lora, train_lora, add_cn):
        ...
        unet.enable_gradient_checkpointing()
            # replace_conv2d_with_sconv2d("conv_in", unet.conv_in, unet)
            # replace_conv2d_with_sconv2d("down_blocks", unet.down_blocks, unet)
