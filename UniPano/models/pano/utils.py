...
from external.spherenet import SphereConv2d, LoRACompatibleSphereConv2d, LoRASphereConv2dLayer, SphereAvgPool2d, SphereMaxPool2d, SphereUpsample2D
from diffusers.models.resnet import Downsample2D, Upsample2D


def toggle_sphere_conv(module, use_sphere_conv : bool):
    if hasattr(module, "use_sphere_conv"):
        module.use_sphere_conv = use_sphere_conv
    for submodule in module.children():
        toggle_sphere_conv(submodule, use_sphere_conv)

def check_sphere_conv(module, use_sphere_conv : bool):
    if hasattr(module, "use_sphere_conv"):
        assert module.use_sphere_conv == use_sphere_conv
    for submodule in module.children():
        check_sphere_conv(submodule, use_sphere_conv)

def vae_set_use_conv(module, use_conv : bool, use_sphere_conv : bool = False):
    if not use_sphere_conv:
        return
    if isinstance(module, Downsample2D):
        module.use_conv = use_conv
    for submodule in module.children():
        vae_set_use_conv(submodule, use_conv, use_sphere_conv)
...
def get_equi_coords(equi_h, equi_w, device, dtype=torch.float32):
...
def get_pers_coords(pers_h, pers_w, device, dtype=torch.float32):
    x, y = np.meshgrid(np.linspace(0, 1, pers_w), np.linspace(0, 1, pers_h))
    xy = np.stack([x, y])
    pers_coords = torch.tensor(xy, device=device, dtype=dtype)
    pers_coords = rearrange(pers_coords, 'c h w -> h w c')
    return pers_coords
